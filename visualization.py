"""Visualization utilities for backtest performance."""
from __future__ import annotations

import logging

import pandas as pd
import plotly.graph_objects as go

logger = logging.getLogger(__name__)

# --- Constants for DataFrame Columns ---
COL_TIMESTAMP = "timestamp"
COL_PORTFOLIO_VALUE = "portfolio_value"
COL_EVENT_TYPE = "event_type"
COL_PRICE = "price"

# --- Constants for Event Types ---
EVENT_TYPE_BUY = "buy_signal"
EVENT_TYPE_SELL = "sell_signal"

# --- Constants for Plot Styling ---
PORTFOLIO_COLOR = "blue"
BUY_COLOR = "green"
SELL_COLOR = "red"
MARKER_SIZE = 10
MARKER_SYMBOL_BUY = "triangle-up"
MARKER_SYMBOL_SELL = "triangle-down"
PLOT_TEMPLATE = "plotly_white"


def _add_trade_markers_to_figure(
    fig: go.Figure, trade_data: list[dict] | None
) -> None:
    """Add trade markers (buy/sell signals) to the Plotly figure.

    Args:
        fig: The Plotly Figure object to add traces to.
        trade_data: Optional list of dicts with timestamp, event_type, price.
    """
    if not trade_data:
        logger.info("No trade data provided; skipping trade markers.")
        return

    trade_df = pd.DataFrame(trade_data)

    if trade_df.empty:
        logger.warning(
            "Trade data was provided but resulted in an empty DataFrame. "
            "No trade markers will be plotted."
        )
        return

    required_trade_cols = {COL_TIMESTAMP, COL_EVENT_TYPE, COL_PRICE}
    if not required_trade_cols.issubset(trade_df.columns):
        missing_cols = required_trade_cols - set(trade_df.columns)
        logger.warning(
            f"Trade data is missing required columns: {missing_cols}. "
            "No trade markers will be plotted."
        )
        return

    # Convert timestamp to datetime objects if valid data is present
    trade_df = pd.to_datetime(trade_df, utc=True)

    buy_trades = trade_df == EVENT_TYPE_BUY]
    sell_trades = trade_df == EVENT_TYPE_SELL]

    if not buy_trades.empty:
        fig.add_trace(
            go.Scatter(
                x=buy_trades,
                y=buy_trades,
                mode="markers",
                name="Buy Trade",
                marker={
                    "symbol": MARKER_SYMBOL_BUY,
                    "size": MARKER_SIZE,
                    "color": BUY_COLOR,
                },
            )
        )

    if not sell_trades.empty:
        fig.add_trace(
            go.Scatter(
                x=sell_trades,
                y=sell_trades,
                mode="markers",
                name="Sell Trade",
                marker={
                    "symbol": MARKER_SYMBOL_SELL,
                    "size": MARKER_SIZE,
                    "color": SELL_COLOR,
                },
            )
        )


def create_performance_chart(
    plot_data: list[dict], trade_data: list[dict] | None = None
) -> go.Figure:
    """Create a Plotly chart from backtest performance data.

    Args:
        plot_data: List of dicts with timestamp, portfolio_value.
        trade_data: Optional list of dicts with timestamp, event_type, price.

    Returns:
        A Plotly Figure object for rendering performance metrics.

    Raises:
        ValueError: If required fields are missing in plot_data.
    """
    if not plot_data:
        logger.warning("Empty plot data provided. Returning a blank chart.")
        fig = go.Figure()
        fig.update_layout(
            title="No Data Available",
            xaxis_title="Time",
            yaxis_title="Value",
            template=PLOT_TEMPLATE,
        )
        return fig

    portfolio_df = pd.DataFrame(plot_data)
    required_plot_cols = {COL_TIMESTAMP, COL_PORTFOLIO_VALUE}

    if not required_plot_cols.issubset(portfolio_df.columns):
        missing_cols = required_plot_cols - set(portfolio_df.columns)
        error_msg = f"Missing required columns in plot_data: {missing_cols}"
        logger.error(error_msg)
        raise ValueError(error_msg)

    portfolio_df = pd.to_datetime(
        portfolio_df, utc=True
    )

    fig = go.Figure()

    # Portfolio value line
    fig.add_trace(
        go.Scatter(
            x=portfolio_df,
            y=portfolio_df,
            mode="lines",
            name="Portfolio Value",
            line={"color": PORTFOLIO_COLOR},
        )
    )

    # Add trade markers using the helper function
    _add_trade_markers_to_figure(fig, trade_data)

    # Final layout updates
    fig.update_layout(
        title="Backtest Performance",
        xaxis_title="Time",
        yaxis_title="Value",
        showlegend=True,
        template=PLOT_TEMPLATE,
    )

    return fig
